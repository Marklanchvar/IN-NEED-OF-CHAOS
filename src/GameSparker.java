...
public class GameSparker extends Applet implements Runnable
{
	...
	public void run() {
		...
		final ContO[] modelList = new ContO[124];
		final CarDefine carDefine = new CarDefine(modelList, medium, trackers, this);
		...
		this.loadbase(modelList, medium, trackers, xtGraphics, false);
		...
		this.readcookies(xtGraphics, carDefine, modelList);
		...
		System.gc();
		byte[] targetMillis = { 47, 30 };
		byte minimumMillis = 15;
		int n7 = 0;
		int n8 = 0;
		int n9 = 0;
		int n10 = 0;
		...
		while (true) {
			...
			if (xtGraphics.fase == -9) {
				...
				if (n8 < 2) {
					...
					xtGraphics.inishcarselect(modelList);
					n8 = 0;
					xtGraphics.fase = 7;
					this.mvect = 50;
					this.mouses = 0;
			...
			if (xtGraphics.fase == -22) {
				checkPoints.name = Madness.testcar;
				checkPoints.stage = -1;
				this.loadstage(array2, modelList, medium, trackers, checkPoints, xtGraphics, array3, record);
			...
			if (xtGraphics.fase == -5) {
				this.mvect = 100;
				xtGraphics.finish(checkPoints, modelList, this.u[0], this.xm, this.ym, this.moused);
				xtGraphics.ctachm(this.xm, this.ym, this.mouses, this.u[0]);
				if (this.mouses == 2) {
					this.mouses = 0;
				}
			...
			if (xtGraphics.fase == 7) {
				xtGraphics.carselect(this.u[0], modelList, array3[0], this.xm, this.ym, this.moused);
				xtGraphics.ctachm(this.xm, this.ym, this.mouses, this.u[0]);
				if (this.mouses == 2) {
					this.mouses = 0;
				}
			...
			if (xtGraphics.fase == 1177) {
				...
				if (n == 0) {
					...
					if (this.mload == 2) {
						carDefine.loadready();
						this.loadbase(modelList, medium, trackers, xtGraphics, true);
						this.readcookies(xtGraphics, carDefine, modelList);
						this.mload = -1;
					}
					System.gc();
					login = new Login(medium, this.rd, xtGraphics, this);
					globe = new Globe(this.rd, xtGraphics, medium, login, carDefine, checkPoints, modelList, array2, this);
				...
				if (login.fase != 18) {
					...
					if (login.fase >= 1 && login.fase <= 11) {
						login.multistart(modelList, this.xm, this.ym, this.moused);
					}
					if (login.fase >= 12 && login.fase <= 17) {
						if (globe.open != 452) {
							login.multimode(modelList);
						}
						else {
							b = true;
						}
					...
					if (lobby.fase == 1) {
						...
						if (globe.open != 452) {
							lobby.lobby(this.xm, this.ym, this.moused, this.mousew, checkPoints, this.u[0], modelList);
						}
						else {
							b2 = true;
						}
						...
						if (lobby.loadstage > 0) {
							...
							this.repaint();
							trackers.nt = 0;
							if (this.loadstagePreview(lobby.loadstage, "", array2, modelList, medium, checkPoints)) {
					...
					if (lobby.fase == 2) {
						...
						if (b3) {
							...
							trackers.nt = 0;
							if (this.loadstagePreview(lobby.gstgn[n31], lobby.gstages[n31], array2, modelList, medium, checkPoints)) {
			...
			if (xtGraphics.fase == 0) {
				for (int n33 = 0; n33 < xtGraphics.nplayers; ++n33) {
					if (array3[n33].newcar) {
						final float xz = array2[n33].xz;
						final float xy = array2[n33].xy;
						final float zy = array2[n33].zy;
						array2[n33] = new ContO(modelList[array3[n33].cn], array2[n33].x, array2[n33].y, array2[n33].z, 0);
						array2[n33].xz = xz;
						array2[n33].xy = xy;
						array2[n33].zy = zy;
				...
				int n34 = 0;
				final Integer[] renderIndices = new Integer[this.nob];
				for (int n35 = 0; n35 < this.nob; ++n35) {
					if (array2[n35].dist != 0) {
						renderIndices[n34++] = n35;
					} else {
						array2[n35].d(this.rd);
					}
				}

				// Replace O(N^2) sort with O(N log N) TimSort
				java.util.Arrays.sort(renderIndices, 0, n34, (a, b) -> Float.compare(array2[b].dist, array2[a].dist));

				// Render sorted objects
				for (int n41 = 0; n41 < n34; ++n41) {
					array2[renderIndices[n41]].d(this.rd);
				}
			...
			if (xtGraphics.fase == 7001) {
				for (int n48 = 0; n48 < xtGraphics.nplayers; ++n48) {
					if (array3[n48].newedcar == 0 && array3[n48].newcar) {
						final float xz2 = array2[n48].xz;
						final float xy2 = array2[n48].xy;
						final float zy2 = array2[n48].zy;
						xtGraphics.colorCar(modelList[array3[n48].cn], n48);
						array2[n48] = new ContO(modelList[array3[n48].cn], array2[n48].x, array2[n48].y, array2[n48].z, 0);
						array2[n48].xz = xz2;
						array2[n48].xy = xy2;
						array2[n48].zy = zy2;
			...
			if (xtGraphics.fase == -1) {
				...
				for (int n79 = 0; n79 < xtGraphics.nplayers; ++n79) {
					...
					if (array2[n79].fcnt == 7 || array2[n79].fcnt == 8) {
						array2[n79] = new ContO(modelList[array3[n79].cn], 0, 0, 0, 0);
			...
			if (xtGraphics.fase == -3) {
				...
				for (int n92 = 0; n92 < xtGraphics.nplayers; ++n92) {
					...
					if (array2[n92].fcnt == 7 || array2[n92].fcnt == 8) {
						array2[n92] = new ContO(modelList[array3[n92].cn], 0, 0, 0, 0);
			...
			case -3:
			case 7001:
				target = targetMillis[0];
				break;
			default:
				target = targetMillis[1];
				break;
			}
			gameTime = endTime - startTime;
			millisToSleep = Math.max((target - gameTime), minimumMillis);
			if (this.exwist) {
				this.rd.dispose();
	...
	public void paint(final Graphics graphics) {
		...
		if (this.lastw != width || this.lasth != height) {
			...
			this.lasth = height;
			this.showsize = 100;
			if (width <= 800 || height <= 550) this.reqmult = 0.0f;
			if (Madness.fullscreen) {
				this.apx = (int) (width / 2.0f - 400.0f * this.apmult);
				this.apy = (int) (height / 2.0f - 225.0f * this.apmult);
			}
		}

		int offsetX = 0, offsetY = 0;
		if (this.moto == 1 && this.shaka > 0) {
			offsetX = (int) ((this.shaka * 2 * Math.random() - this.shaka) * this.apmult);
			offsetY = (int) ((this.shaka * 2 * Math.random() - this.shaka) * this.apmult);
			--this.shaka;
		}
		...
		if (!Madness.fullscreen) {
			if (this.showsize != 0) {
				...
				float n3 = Math.min((width - 40) / 800.0f - 1.0f, (height - 70) / 450.0f - 1.0f);
				this.apmult = 1.0f + Math.max(0.0f, Math.min(1.0f, n3)) * this.reqmult;

				g2d.drawImage(this.carmaker[this.oncarm ? 1 : 0], 50, 14, this);
				g2d.drawImage(this.stagemaker[this.onstgm ? 1 : 0], width - 208, 14, this);
				g2d.drawImage(this.sizebar, width / 2 - 230, 23, this);
				g2d.drawImage(this.blb, (int) (width / 2 - 222 + 141.0f * this.reqmult), 23, this);
				g2d.drawImage(this.chkbx[this.smooth], width / 2 - 53, 23, this);

				g2d.setFont(new Font("Arial", Font.BOLD, 11));
				g2d.setColor(new Color(74, 99, 125));
				g2d.drawString("Screen Size:", width / 2 - 224, 17);
				g2d.drawString("Smooth", width / 2 - 36, 34);

				g2d.drawImage(this.fulls, width / 2 + 27, 15, this);
				g2d.setColor(new Color(94, 126, 159));
				g2d.drawString("Fullscreen", width / 2 + 63, 30);

				g2d.drawImage(this.chkbx[Madness.anti], width / 2 + 135, 9, this);
				g2d.drawString("Antialiasing", width / 2 + 152, 20);
				g2d.drawImage(this.chkbx[this.moto], width / 2 + 135, 26, this);
				g2d.drawString("Motion Effects", width / 2 + 152, 37);

				g2d.setColor(Color.BLACK);
				g2d.fillRect(width / 2 - 153, 5, 80, 16);
				g2d.setColor(new Color(121, 135, 152));
				String sizeStr = (this.reqmult == 0.0f) ? "Original" : (this.reqmult == 1.0f) ? "Maximum" : (int) (this.apmult * 100.0f) + "%";
				g2d.drawString(sizeStr, width / 2 - 150, 17);

				if (!this.oncarm && !this.onstgm) --this.showsize;

				if (this.showsize == 0) {
					g2d.setColor(Color.BLACK);
					g2d.fillRect(width / 2 - 260, 0, 520, 40);
					g2d.fillRect(50, 14, 142, 23);
					g2d.fillRect(width - 208, 14, 158, 23);
				}
			}

			this.apx = (int) (width / 2.0f - 400.0f * this.apmult);
			this.apy = Math.max(50, (int) (height / 2.0f - 225.0f * this.apmult - 50.0f));
		}

		if (!Madness.fullscreen && this.apmult > 1.0f && this.smooth == 1) {
			g2d.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		}

		int renderX = this.apx + (this.moto == 1 ? offsetX : 0);
		int renderY = this.apy + (this.moto == 1 ? offsetY : 0);

		if (this.moto == 1) {
			g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, this.mvect / 100.0f));
			this.rd.setRenderingHint(RenderingHints.KEY_ALPHA_INTERPOLATION, RenderingHints.VALUE_ALPHA_INTERPOLATION_SPEED);
		}

		if (!Madness.fullscreen && this.apmult > 1.0f) {
			g2d.drawImage(this.offImage, renderX, renderY, (int) (800.0f * this.apmult), (int) (450.0f * this.apmult), this);
		} else {
			g2d.drawImage(this.offImage, renderX, renderY, this);
		}

		if (this.moto == 1) {
			this.cropit(g2d, offsetX, offsetY);
		}
	}
		...
		try {
			...
			while ((line = dataInputStream.readLine()) != null) {
				...
				if (string.startsWith("chk")) {
					...
					getint6 += 46;
					float getint7 = medium.ground - array2[getint6].grat;
					if (getint6 == 110) {
	...
	public void loadbase(final ContO[] array, final Medium medium, final Trackers trackers, final xtGraphics xtGraphics, final boolean loadOnline) {
